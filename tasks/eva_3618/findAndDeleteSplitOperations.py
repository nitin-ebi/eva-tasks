...
def find_and_delete_split_operations(private_config_xml_file):
    ...
    with get_mongo_connection_handle("production_processing", private_config_xml_file) as mongo_conn:
        ...
        logger.info(f'CVE OPS IDS : {cve_ops_ids}')
        logger.info(f'CVE OPS ACCS : {cve_ops_accs}')
        ...
        logger.info(f'SVE OPS IDS : {sve_ops_ids}')

        # delete sve operations
        sve_del_query = {'_id': {'$in': list(sve_ops_ids)}}
        sve_coll = mongo_conn['eva_accession_sharded']['submittedVariantOperationEntity']
        sve_coll.delete_many(sve_del_query, no_cursor_timeout=True)
        dbsnp_sve_coll = mongo_conn['eva_accession_sharded']['dbsnpSubmittedVariantOperationEntity']
        dbsnp_sve_coll.delete_many(sve_del_query)

        # delete cve operations
        cve_del_query = {'_id': {'$in': list(cve_ops_ids)}}
        cve_coll = mongo_conn['eva_accession_sharded']['clusteredVariantOperationEntity']
        cve_coll.delete_many(cve_del_query, no_cursor_timeout=True)
        dbsnp_cve_coll = mongo_conn['eva_accession_sharded']['dbsnpClusteredVariantOperationEntity']
        dbsnp_cve_coll.delete_many(cve_del_query, no_cursor_timeout=True)
...
if __name__ == "__main__":
    ...
    find_and_delete_split_operations(args.private_config_xml_file)
